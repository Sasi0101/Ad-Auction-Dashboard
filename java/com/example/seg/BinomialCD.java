...
public class BinomialCD {
    ...
    /**
     * Constructor + implementing functionality for Poisson CD
     */
    public BinomialCD (){
        //creating the scene
        ...
        stage.setTitle("Binomial Cumulative Distribution Calculator");
        ...
        calculate.setOnAction(e -> {
            ...
            if (checkInt()) {
                ...
                if (n > 5000) {
                    ...
                    Answer = binomialCdCalc(x, n, p).round(new MathContext(11, RoundingMode.HALF_UP));
    ...
    /**
     * method to get the cumulative answer
     * @param x the number of successes
     * @param n the number of total trials
     * @param p the probability of a single success
     * @return BigDecimal result
     */
    protected static BigDecimal binomialCdCalc(int x, int n, BigDecimal p){
        Functions functions = new Functions();
        BigDecimal result = BigDecimal.valueOf(0);
        for (int X=0;X<=x;X++) {
            result = result.add(functions.binomialPdCalc(X,n,p));
        }
        return result;
    }
