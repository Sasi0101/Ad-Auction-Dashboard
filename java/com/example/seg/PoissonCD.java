...
public class PoissonCD {
    ...
    public PoissonCD(){
        ...
        stage.setTitle("Poisson Cumulative Distribution Calculator");
        ...
        calculate.setOnAction(e -> {
            ...
            if (checkInt()) {
                ...
                if (x > 5000) {
                    ...
                    Answer = poissonCdCalc(x,l).round(new MathContext(7, RoundingMode.HALF_UP));
     ...
     */
    protected static BigDecimal poissonCdCalc(int x, BigDecimal l){
        Functions functions = new Functions();
        BigDecimal result = BigDecimal.valueOf(0);
        for (int X=0;X<=x;X++) {
            System.out.println("Iteration: " + X);
            result = result.add(functions.poissonPdCalc(X,l));
        }
        return result;
    }
    ...
    public boolean checkInt() {
        return numSuccesses.matches("\\d+\\d*") && meanSuccesses.matches("\\d+\\.\\d*");
    }
}
